...
class request {
	...
	public function getString($name, $validator = null, $defaultValue = null, $stripWhite = true) {
		...
		if (!is_null($validator) && method_exists('Validate', 'is'.$validator)) {
			if (!call_user_func(array('Validate', 'is'.$validator), $value)) {
				$this->validateErrors[$name] = $validator;	
			}
		}
		return $value;
	}
	...
	public function getDate($name, $validator = null) {
		...
		if(empty($day) && empty($month) && empty($year) && empty($hour) && empty($minute)) return false;
