...
class AbstractTable {
	...
	function GetByFields($data = null, $orders = null, $limited = true) {
		...
		foreach ($this->columns as $column) {
			if (isset($data[$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . '=' . $this->prepareColumnValue($column, $data[$column['name']]);
			} elseif (!empty($data['LIKE'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' LIKE(' . $this->prepareColumnValue($column, '%'.$data['LIKE'.$column['name']].'%') . ')';
			} elseif (!empty($data['IN'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' IN (' . $data['IN'.$column['name']] . ')';
			} elseif (!empty($data['GT'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' > ' . $data['GT'.$column['name']];
			} elseif (!empty($data['LT'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' < ' . $data['LT'.$column['name']];
			} elseif (!empty($data['FROM'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' >= ' . $data['FROM'.$column['name']];
			} elseif (!empty($data['TO'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' <= ' . $data['TO'.$column['name']];
			} elseif (!empty($data['NOT'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				$whereClause .= $column['name'] . ' != ' . $data['NOT'.$column['name']];
			} elseif (!empty($data['NOTIN'.$column['name']])) {
				if (strlen($whereClause)) $whereClause .= ' AND ';
				//$value = (is_array())
				$whereClause .= $column['name'] . ' NOT IN (' . $data['NOTIN'.$column['name']] . ')';
			}
		}
		if (strlen($whereClause)) $whereClause = " WHERE " . $whereClause;
		if (is_array($orders)) {
	...
	function &GetReader($data = null, $orders = null, $limitCount = null, $limitOffset = null) {
		...
		$SQL = sprintf("
		SELECT SQL_CALC_FOUND_ROWS * FROM %s%s%s%s", $this->tableName, $whereClause, $orderClause, $limitClause);
	...
	function UpdateByFields(&$data, $where) {
		$keyColumn = $this->getKeyColumn();
		if (is_array($keyColumn)) {
			$this->OnBeforeUpdate($data);
			$SQL = "";
			...
			$whereClause = $this->Convert($where);
			$SQL = sprintf("UPDATE %s SET %s WHERE %s", $this->tableName, $SQL, $whereClause);
			$this->connection->ExecuteNonQuery($SQL);
			$this->OnAfterUpdate($data);
		}
...
//		}
	...
	function buildPager($page, $itemsCount, $itemsPerPage) {
		...
		$pagesPerPager = DEFAULT_ITEMSPERPAGE;
