...
class mssqlconnectionproperties {
	...
	function mssqlconnectionproperties($host, $user, $password, $database) {
		$this->host = $host;
		$this->user = $user;
	...
	function toURIString() {
		return 'mssql://'.$this->getUser().':'.$this->getPassword().'@'.$this->getHost().'/'.$this->getDatabase();
	}
	...
	public static function createByURI( $stringURI ) {
		...
		return new mssqlconnectionproperties($host, $user, $password, $database);
	}
}