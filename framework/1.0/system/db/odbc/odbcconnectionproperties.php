...
class odbcconnectionproperties {
	...
	function odbcconnectionproperties($path,$user, $password) {
		$this->user = $user;
	...
	function toURIString() {
		return 'odbc://'.$this->getUser().':'.$this->getPassword().'@/'.$this->getPath();
	}
	...
	public static function createByURI( $stringURI ) {
		...
		$path = substr($params["path"], 1, strlen($params["path"]));
		
		return new odbcconnectionproperties($path, $user, $password);
	}
}