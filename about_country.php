...
class IndexPage extends PublicPage {
	...
	function render() {
		...
		// get special offers
		$spOff = $this->specialOffersTable->GetList($this->sfilter);
				...
				if ($value['intCountryID'] == $tmp[$i]) {
					foreach ($countries as $k => $val) {
						if ($val['intCountryID'] == $value['intCountryID']) {
							$value['ext'] = substr($value['varFile'], -4);
							$resArr[$val['varName']][] = $value;
						}
					}
				}	
			}
...
Kernel::ProcessPage(new IndexPage("about_country.tpl"));