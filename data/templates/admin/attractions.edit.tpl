...
{literal}
	...
	function setRegionResort(){
		...
		$('#intResortID option.first').css('display','');

		if($('#intResortID option:selected').css('display')=='none'){
			$('#intResortID option').removeAttr('selected');
		}
...
<h1>{$pagetitle}</h1>
<input type="button" class="iconize" rel="78" value="Вернуться к списку" onclick='Go("attractions.php")'/>

<form action="attractions.edit.php" method="POST" id="editForm" name="editForm">
<input type="hidden" name="event" id="event" value="" />
<input type="hidden" name="intAttractionID" id="intAttractionID" value="{$data.intAttractionID}" />

<table width="100%" class="container"><tr><td valign="top">
	<table class="bordered" width="100%">
		<thead><tr><th colspan="2">Данные достопримечательности</th></tr></thead>
		<tbody>
			<tr>
			...
			<tr>
				...
				<td>
					<select name="intResortID" id="intResortID">
						...
						<option value="{$item.intResortID}" rel="{$item.intCountryID}" {if $data.intCountryID != $item.intCountryID}style="display:none"{/if} {if $data.intResortID==$item.intResortID} selected="selected"{/if}>{$item.varName}</option>
						{/foreach}
					</select>
				</td>
			</tr>
