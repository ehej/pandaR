...
<table class="bordered">
	...
	<tr {if $item.varStatus=='confirmed'}Подтверждена{elseif $item.varStatus=='pending'} style="background:#FFCC99;"{elseif $item.varStatus=='denial'} style="background:#FFB6C1;"{/if}>
		...
		<td nowrap="nowrap">
		<input type="button" class="iconize" rel="83" value="Удалить" onclick='OnDelete("applications.php?intOrderID={$item.intOrderID}&event=delete", "Вы уверены, что хотите удалить эту заявку?")'/>
		</td>
	</tr>
	{/if}{foreachelse}
