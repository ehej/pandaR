...
<form action="tourtypes.php" method="POST" id="sort_menu">
	...
	<div id="manage_menu_view">
		...
		{foreach from=$tourtypes_list item=item}
			<li id="{$item.intTypeID}">
			<table cellspacing="0" cellpadding="0">
				<tr>
					<td><span style="float:left;"><a href="tourtypes.edit.php?intTypeID={$item.intTypeID}" title="{$item.varName}">{$item.varName}</a></td>
					<td><span class="iconset iconize" style="margin-left: 20px; cursor: pointer; float:right;" rel="83" onclick='javascript:OnDelete("tourtypes.php?intTypeID={$item.intTypeID}&event=delete", "Вы уверены, что хотите удалить запись с ID={$item.intTypeID}?")'></span></td>
				</tr>
			</table>		
			</li>
		{/foreach}
