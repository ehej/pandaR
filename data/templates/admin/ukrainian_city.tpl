<h1>{$pagetitle}</h1>
<input type="button" class="iconize" rel="23" value="Добавить" onclick='Go("ukrainian_city.edit.php")'/>

<form action="ukrainian_city.php" method="GET" id="searchForm">
...
<!-- Таблица -->
	...
	<tr onDblClick='javascript:Go("ukrainian_city.edit.php?intCityID={$item.intCityID})"'>
		<td>{$item.varName}</td>
		<td>{if $item.isActive == 1}Да{else}Нет{/if}</td>
		<td nowrap="nowrap">
			<input type="button" class="iconize" rel="52" value="Редактировать" onclick='javascript:Go("ukrainian_city.edit.php?intCityID={$item.intCityID}")'/> 
			<input type="button" class="iconize" rel="83" value="Удалить" onclick='javascript:OnDelete("ukrainian_city.php?intCityID={$item.intCityID}&event=delete", "Вы уверены, что хотите удалить запись с ID={$item.intCityID}?")'/>
		</td>
	</tr>
	{/if}
	{foreachelse}
