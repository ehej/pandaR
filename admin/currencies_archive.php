...
class IndexPage extends AdminPage {
	...
	function index() {
		...
		$this->setPageTitle('Архив курса валют');
		$this->setBoldMenu('currenciesarchive');

		$this->applicationsTable = new CurrenciesArchiveTable($this->connection);
	...
	function setFilters() {
		...
		$this->sfilter['sortBy'] = $this->request->getString('sortBy', null);
	...
	function OnDelete() {
		...
		$app['intArchiveID'] = $this->request->getNumber('intArchiveID');
		$this->applicationsTable->Delete($app);

		$this->response->redirect('currencies_archive.php?page='.$this->page);
	}
...
Kernel::ProcessPage(new IndexPage("currencies_archive.tpl"));